...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // Find first space, second space and '\r\n'
            char* firstspace = strchr(line, ' ');
            char* secondspace = strchr(firstspace + 1, ' ');
            char* clrf = strstr(secondspace + 1, "\r\n" );
            
            if (firstspace == NULL || secondspace == NULL || clrf == NULL )
            {
                error(400);
                continue;
            } 
            
            // Create 'method' // Use strncpy to fill // Set the last char to '\0'
            // char* strncpy(char* dest, const char* src, size_t n);
            char method [firstspace - line + 1];
            strncpy(method,line, firstspace - line);
            method[firstspace - line] = '\0';
            // debugging
            printf("method = %s\n",method);    
            
            // create request target // Use strncpy to fill // Set the last char to '\0'
            char reqTar[secondspace - firstspace];
            strncpy( reqTar, (firstspace + 1), secondspace - (firstspace + 1));
            reqTar[secondspace - (firstspace + 1)] = '\0';
            // debugging
            printf("request target = %s\n",reqTar);    
        
            // Create protocolversion // Use strncpy to fill // Set the last char to '\0'
            char prVersion [clrf - secondspace];
            strncpy(prVersion, secondspace + 1, clrf - secondspace + 1);
            prVersion[clrf - (secondspace + 1)] = '\0';
            // debugging
            printf("Protocol version = %s\n",prVersion);
            for (int i = 0, n = strlen(prVersion); i < n; i++)
            {
                printf("%i:'%i' ", i,prVersion[i]);
            }
            printf("\n");
            
            // Find absolute-path
            // Find query-start'?' -> if not found -> return pointer to '\0' with strchrnul
            char* queryStart = strchrnul(reqTar , '?');
            char* queryEnd = strchr (reqTar, '\0');
 
            char absolutePath [queryStart - reqTar + 1];
            strncpy(absolutePath, reqTar, queryStart - reqTar);
            absolutePath[queryStart - reqTar] = '\0';
            // debugging
            printf("absolute-path = %s\n",absolutePath);

            
         
            // Validate methode. if not get, reply 405 and continue.
            ...
            // Validate requestTarget. If not correct, reply 501 and continue
            if (reqTar[0] != '/')
            {
                error(501);
                continue;
            ...
            // Validate ProtocolVersion. If not correct, reply 505 and continue.
            if (strcmp("HTTP/1.1", prVersion) != 0)
            {
                for (int i = 0, n = strlen(prVersion); i < n; i++)
                {
                    printf("%i:'%i' ", i,prVersion[i]);
            ...
            // If no '.', reply 501 and continue.
            if (strchr(absolutePath, '.') == NULL)
            {
                printf("strchr(absolutePath, '.') == NULL");
                error(501);
                continue;
            }

            // TODO: Re-define a string called query that contains the query substring from request-target. 
            // If the latter is absent (even if a ? is present), then the string should be "", thereby consuming one byte
            // whereby query[0] is '\0'.
            ...
            {
                n = (queryEnd - queryStart);
                queryStart = queryStart + 1;
            }
            
            char query[n];
            strncpy (query, queryStart, n);
          
            // TODO: concatenate (= joining character strings end-to-end.) root and absolute-path
